...
/* line gradients added by tobozo nov 2019
 *
 */

RGBColor TFT_eFEX::colorAt( int32_t x0, int32_t y0, int32_t x1, int32_t y1, int32_t x2, int32_t y2, RGBColor colorstart, RGBColor colorend ) {
  float linedistance;
  float pixeldistance;
...
void TFT_eFEX::drawGradientLine( int32_t x0, int32_t y0, int32_t x1, int32_t y1, RGBColor colorstart, RGBColor colorend ) {

  if( colorstart == colorend ) {
    // be lazy
    _tft->drawLine( x0, y0, x1, y1, _tft->color565(colorstart.r, colorstart.g, colorstart.b) );
    return;
  }
    ...
    for (; x0 <= x1; x0++) {
      ...
      if (err < 0) {
        ...
        if (dlen == 1) {
          ...
          _tft->drawPixel(y0, xs, _tft->color565( _color.r, _color.g, _color.b ) );
        } else {
    ...
    for (; x0 <= x1; x0++) {
      ...
      if (err < 0) {
        ...
        if (dlen == 1) {
          ...
          _tft->drawPixel(xs, y0, _tft->color565( _color.r, _color.g, _color.b ) );
        } else {
